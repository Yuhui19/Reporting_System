...
//        response.setHeader("Content-Disposition", "attachment; filename=\"name_of_excel_file.xls\""); // TODO: File name cannot be hardcoded here
    ...
    @PutMapping("/excel/{id}")
    @ApiOperation("update excel file in S3 and dynamo")
    public ResponseEntity<ExcelResponse> updateExcel(@RequestBody @Validated ExcelRequest request, @PathVariable String id) {
        log.debug("Got Request to update Single Sheet Excel:{}", id);
        ExcelFile fileInfo = excelService.updateFile(request, id, false);
        ExcelResponse response = new ExcelResponse();
        BeanUtils.copyProperties(fileInfo, response);
        response.setFileDownloadLink(this.generateFileDownloadLink(fileInfo.getFileId()));
        response.setFileLocation(fileInfo.getFileLocation());
