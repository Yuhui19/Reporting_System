...
//        entity.setExcelReport(excelReport);
        entity.setReqId(request.getReqId());
        entity.setCreatedTime(LocalDateTime.now());
        convertRequestToEntity(entity, request);

        return reportRequestRepo.save(entity);
    }

    private void convertRequestToEntity(ReportRequestEntity entity, ReportRequest request) {
//        entity.setReqId(request.getReqId());
        entity.setSubmitter(request.getSubmitter());
        entity.setDescription(request.getDescription());
//        entity.setCreatedTime(LocalDateTime.now());
...
//        PDFResponse pdfResponse = new PDFResponse();
        ...
        executorService.execute(() -> {
            ExcelResponse excelResponse = new ExcelResponse();
            try {
                excelResponse = restTemplate.postForEntity("http://localhost:8888/excel", request, ExcelResponse.class).getBody();
            } catch(Exception e){
        ...
        executorService.execute(() -> {
            PDFResponse pdfResponse = new PDFResponse();
            try {
                pdfResponse = restTemplate.postForEntity("http://localhost:9999/pdf", request, PDFResponse.class).getBody();
            } catch(Exception e){
...
//    @Transactional // why this? email could fail
    public void updateAsyncPDFReport(SqsResponse response) {
        ...
        String to = "yuhuizhang1995@gmail.com";
...
//            }
    ...
    @Override
    public void deleteEntity(String reqId) {
        ReportRequestEntity entity = reportRequestRepo.findById(reqId).orElseThrow(RequestNotFoundException::new);
        String excelId = entity.getExcelReport().getFileId();
        String pdfId = entity.getPdfReport().getFileId();
//        RestTemplate rs = new RestTemplate();
        ExecutorService executorService = Executors.newFixedThreadPool(2);
        executorService.execute(() -> {
            try {
                restTemplate.delete("http://localhost:8888/excel/" + excelId);
            } catch(Exception e){
                log.error("Excel Delete Error : e", e);
            }
        });
        executorService.execute(() -> {
            try {
                restTemplate.delete("http://localhost:9999/pdf/" + pdfId);
            } catch(Exception e){
                log.error("PDF Delete Error : e", e);
            }
        });
    ...
    public ReportVO getReportById(String reqId) {
        return new ReportVO(reportRequestRepo.findById(reqId).get());
    }

    @Override
    public ReportVO updateReportsSync(ReportRequest request, String reqId) {

        request.setReqId(reqId);
        ReportRequestEntity temp = reportRequestRepo.findById(reqId).get();
        temp.setUpdatedTime(LocalDateTime.now());

        temp.setSubmitter(request.getSubmitter());
        temp.setDescription(request.getDescription());

        temp.getPdfReport().setUpdatedTime(LocalDateTime.now());
        temp.getPdfReport().setStatus(ReportStatus.PENDING);
        temp.getExcelReport().setUpdatedTime(LocalDateTime.now());
        temp.getPdfReport().setStatus(ReportStatus.PENDING);

        reportRequestRepo.save(temp);

        String excelId = temp.getExcelReport().getFileId();
        String pdfId = temp.getPdfReport().getFileId();
//        RestTemplate rs = new RestTemplate();
        ExecutorService executorService = Executors.newFixedThreadPool(2);
        executorService.execute(() -> {
            ExcelResponse excelResponse = new ExcelResponse();
            try {
                restTemplate.put("http://localhost:8888/excel/" + excelId, request);
                excelResponse.setReqId(reqId);
                excelResponse.setFailed(false);
            } catch(Exception e){
                log.error("Excel Delete Error : e", e);
                excelResponse.setReqId(request.getReqId());
                excelResponse.setFailed(true);
        ...
        executorService.execute(() -> {
            PDFResponse pdfResponse = new PDFResponse();
            try {
                restTemplate.put("http://localhost:9999/pdf/" + pdfId, request);
                pdfResponse.setReqId(reqId);
                pdfResponse.setFailed(false);
            } catch(Exception e){
                log.error("PDF Delete Error : e", e);
                pdfResponse.setReqId(request.getReqId());
                pdfResponse.setFailed(true);
