...
function showDelete(reqId){
    if(confirm("Are you sure to delete report?")){
        ...
        var req = new XMLHttpRequest();
        req.open("DELETE", '/report/'+reqId, false);
        req.responseType = "blob"
        req.onload = function (event) {
            console.info(event);
            if (req.status === 200) {
...
function actionLinks(ps, es, id) {
    return (ps === 'COMPLETED'?"<a onclick='downloadPDF(\""+id+"\")' href='#'>Download PDF</a>":"")
        + (es === 'COMPLETED'?"<a onclick='downloadExcel(\""+id+"\")' style='margin-left: 0.5em' href='#'>Download Excel</a>":"")
        + ("<a onclick='showDelete(\""+id+"\")' style='margin-left: 0.5em' href='#'>Delete</a>")
        + (ps === 'COMPLETED' || es === 'COMPLETED' ? "<a onclick='edit(\""+id+"\")' style='margin-left: 0.5em' href='#'>Update</a>":"");
}
